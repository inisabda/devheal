...
<div class="page-content">
	<div class="row">
		<div class="col-6"><h4><i class="fas fa-file-invoice-dollar"></i> Data Penjualan Obat dan Alkes</h4></div>
	...
	<div class="table-container">
	    ...
	    <tbody>
	    	<?php 
	    		$no = 1;
	    		$query_tampil = "SELECT * FROM tbl_penjualan INNER JOIN tbl_penjualandetail ON tbl_penjualan.no_penjualan = tbl_penjualandetail.no_penjualan INNER JOIN tbl_pegawai ON tbl_penjualan.id_peg = tbl_pegawai.id_peg ORDER BY tbl_penjualan.tgl_penjualan DESC, tbl_penjualan.no_penjualan DESC";
    	 	...
    	 	<tr>
    	 		...
    	 		<td><?php echo date('d-m-Y',strtotime($data_pjl['tgl_penjualan'])); ?></td>
    	 		<td><?php echo $data_pjl['nama_peg']; ?></td>
    	 		<td><?php echo $data_pjl['jml_jual']; ?></td>
    	 		<td class="text-right">Rp. <?php echo number_format($data_pjl['total_penjualan'],2,',','.'); ?></td>
	 				<td class="td-opsi">
            ...
            <a href="laporan/nota_penjualan.php?no_pjl=<?php echo $data_pjl['no_penjualan']; ?>" target="_blank">
              <button class="btn-transition btn btn-outline-dark btn-sm" title="cetak" id="tombol_cetak" name="tombol_cetak">
                  <i class="fas fa-print"></i>
    	 			...
    	 			<?php if($_SESSION['posisi_peg'] == 'Admin' || $_SESSION['posisi_peg'] == 'Manager' || $_SESSION['posisi_peg'] == 'Apoteker') { ?>
              <button class="btn-transition btn btn-outline-danger btn-sm" title="hapus" id="tombol_hapus" name="tombol_hapus" data-nopjl="<?php echo $data_pjl['no_penjualan']; ?>" data-jml="<?php echo $data_pjl['jml_jual']; ?>">
		...
		Swal.fire({
          ...
          if (hapus.value) {
            $.ajax({
              ...
              data: "id="+no_pjl+"&jml_jual="+jml_jual,
