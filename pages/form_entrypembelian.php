...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb bg-light">
    ...
    <li class="breadcrumb-item active" aria-current="page"><i class="fas fa-align-left"></i> Form Entry Data Pembelian Obat / Alkes</li>
  </ol>
</nav>

<div class="page-content">
	<div class="row">
		<div class="col-6"><h4><i class="fas fa-file-invoice-dollar"></i> Form Entry Data Pembelian Obat / Alkes</h4></div>
		<div class="col-6 text-right">
            <a href="?page=tambah_datasupplier">
                <button class="btn btn-sm btn-danger"><i class="fas fa-plus-circle"></i> Tambah Supplier</button>
            </a>
			<a href="?page=datapembelian">
				<button class="btn btn-sm btn-info"><i class="fas fa-file"></i> Data Pembelian</button>
			</a>

            <a href="?page=laporpembelian_apotek">
                <button class="btn btn-sm btn-warning"><i class="fas fa-file"></i> Laporan Pembelian</button>
            </a>
		</div>
        <div class="col-12" id='blink'>
            <b>Pemberitahuan : Obat yang di beli dari Supplier otomatis disimpan pada STOK FARMASI</font></b>
        </div>
	</div>
			...
			<div class="col-md-12 vertical-form">
                ...
                <form method="post" id="form-pembelian" autocomplete="off">
                    <div class="row data-pembelian">
                        ...
                        <div class="position-relative form-group col-md-4">
                            <label for="tgl_pembelian" class="">Tanggal Pembelian Obat</label>
                    ...
                    <div class="row kotak-form-tabel">
                    	<div class="col-md-3 kotak-form-obat-terjual" style="display: ;">
                			...
                			<div class="position-relative form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control form-control-sm" id="kode_obat" placeholder="Kode tidak boleh sama" autofocus onInput="checkKode();">
                            ...
                            <div class="position-relative form-group">
                                ...
                                <div class="input-group">
                                    <input name="nm_obat" id="nm_obat" placeholder="Jenis obat baru / cari database" type="text" class="form-control form-control-sm">
                                    <div class="input-group-append">
                                    ...
                                    <input type="hidden" name="ktg_obat" id="ktg_obat"class="form-control form-control-sm" value="GENERIK">
                                    <input type="hidden" name="bnt_obat" id="bnt_obat"class="form-control form-control-sm">
                                    <input type="hidden" name="minstk_obat" id="minstk_obat"class="form-control form-control-sm" value="20">
                                </div>
                            </div>
                            <div class="position-relative form-group">
                                <label for="no_batch" class="">Nomor Batch</label>
                                <input type="text" class="form-control form-control-sm" id="no_batch" name="no_batch" placeholder="Masukkan No Batch">
                            </div>
                            <div class="position-relative form-group">
                            ...
                            <div class="position-relative form-group text-right mt-2 mb-2">
                                <button type="button" class="btn btn-danger btn-sm" id="reset_obat"><i class="fas fa-undo"></i> reset</button>
                                <button type="button" class="btn btn-info btn-sm" id="tambah_obat"><i class="fas fa-plus-circle"></i> tambah</button>
                            </div>
                    	</div>
                            ...
                            <table class="table display tabel-data">
                                ...
                                <tfoot>
                                    ...
                                    <tr class="baris_total" style="display: none;">
                                        <td colspan="8" class="text-right" style="font-weight: bold;">Total : Rp. <span id="total_pembelian"></span><input type="hidden" name="hidden_totalpembelian" id="hidden_totalpembelian"></td>
                                        <td class="td-opsi">
                        ...
                        <tr>
                            ...
                            <td class="td-opsi">
                                <button class="btn-transition btn btn-outline-dark btn-sm" title="pilih" id="tombol_pilihobat" name="tombol_pilihobat" data-dismiss="modal"                                    
                                    ...
                                    data-no_batch="<?php echo $data['no_batch']; ?>"
                                    data-ktg="<?php echo $data['ktg_obat']; ?>"
                                    data-bnt="<?php echo $data['bnt_obat']; ?>"
                                    data-harga="<?php echo $data['hrgbeli_obat']; ?>"
                                    data-satuan="<?php echo $data['sat_obat']; ?>">Pilih
                                </button>
                            </td>
                        </tr>
                        <?php } ?>
...
<script type="text/javascript">
    function checkKode() {        
        jQuery.ajax({
        url: "ajax/cek_kodeobat_penjualanobat.php",
        data:'kode_obat='+$("#kode_obat").val(),
        type: "POST",
...
$(document).ready(function() {
    ...
    $("#no_batch").keyup(function() { no_batch(); });
    $("#no_batch").change(function() { no_batch(); });
    ...
    $("#ktg_obat").keyup(function() { ktg_obat(); });
    $("#ktg_obat").change(function() { ktg_obat(); });
    $("#bnt_obat").keyup(function() { bnt_obat(); });
    $("#bnt_obat").change(function() { bnt_obat(); });
    ...
    $("#tambah_obat").click(function() {
        ...
        if(kode == ""){
            ...
            output += '<td>'+no_batch+' <input type="hidden" name="hidden_no_batch[]" id="td_no_batch'+count+'" class="td_no_batch" value="'+no_batch+'"></td>';
            output += '<td>'+ktg+' <input type="hidden" name="hidden_ktgobat[]" id="td_ktgobat'+count+'" class="td_ktgobat" value="'+ktg+'"></td>';
            output += '<td>'+exp+' <input type="hidden" name="hidden_expobat[]" id="td_expobat'+count+'" class="td_expobat" value="'+exp+'"></td>';
            output += '<td class="text-right">Rp. '+hrg+' <input type="hidden" name="hidden_hrgobat[]" id="td_hrgobat'+count+'" class="td_hrgobat" value="'+hrg+'"></td>';
            ...
            output += '<td class="text-right">Rp. '+subt+' <input type="hidden" name="hidden_subtotal[]" id="td_subtotal'+count+'" class="td_subtotal" value="'+subt+'"></td>';
                        ...
                        success:function(data) {
                            Swal.fire({
                              ...
                              if (ok.value) {
                                ...
                                if(cr_byr=="Langsung") {
                                    window.open("laporan/nota_pembelianoke.php?no_faktur="+no_faktur);
                                }
                                location.reload();
                              }
                            })
