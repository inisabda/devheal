...
include "../config/setting.php";
    ...
    $query_daft = mysqli_query($mysqli,"SELECT * from tbl_suratijin WHERE no_daftar ='$no_daftar' ");
    while ($dat = mysqli_fetch_assoc($query_daft)) {
    $sip = $dat['sip'];
    $nama = $dat['nama_pas'];
    $istirahat =$dat['istirahat'];
    ...
    $mulai = date("d-m-Y", strtotime($dat['mulai_tanggal']));
    $akhir = date("d-m-Y", strtotime($dat['akhir_tanggal']));
    $istirahat =$dat['istirahat'];
    $alm_pas =$dat['alm_pas'];
        ...
        <tbody>
            <tr>
                <td style="text-align:justify;" colspan="3" width="720">
                    ...
                    <br>Yang bersangkutan benar - benar <b>Sakit</b>, sehingga memerlukan istirahat selama <?php echo $istirahat; ?> hari,
                    <br>terhitung mulai tanggal <b><?php echo tgl_eng_to_ind($mulai); ?></b> s/d tanggal <b><?php echo tgl_eng_to_ind($akhir); ?></b>.
...
<?php
$filename="Surat Ijin.pdf"; //ubah untuk menentukan nama file pdf yang dihasilkan nantinya
//==========================================================================================================
